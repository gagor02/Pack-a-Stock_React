...
export default function CategoriesPage() {
  ...
  const handleDelete = async (category: any) => {
    ...
    try {
      const response = await api.get(`/materials/categories/${category.id}/check_delete/`)
      setDeleteConfirm({
        id: category.id,
        name: category.name,
      ...
      setDeleteConfirm({ id: category.id, name: category.name, materialsCount: 0, materialNames: [] })
    } finally {
                  ...
                  <div className="p-4 bg-amber-500/10 border-2 border-amber-500/30 rounded-xl space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-sm font-medium text-amber-300">
                        {deleteConfirm.materialsCount} material(es) pertenecen a esta categoría
                      </p>
                    </div>
                    ...
                    <p className="text-xs text-amber-400/80 pl-7">
                      Estos materiales quedarán sin categoría asignada.
                    </p>
                  </div>
                )}
